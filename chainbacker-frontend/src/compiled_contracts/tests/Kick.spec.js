...
describe('Crowdfunding System Tests', () => {
    ...
    beforeEach(async () => {
        ...
        usdtMaster = blockchain.openContract(await JettonMinter.createFromConfig({
        ...
        kickFactory = blockchain.openContract(await KickFactory.createFromConfig({
            kickCode,
            backCode: backerCode
        }, kickFactoryCode));
        await kickFactory.sendDeploy(deployer.getSender(), toNano('0.1'));
        let expiration = BigInt(Math.floor(Date.now() / 1000) + KICK_DURATION);
        await kickFactory.sendKick(creator.getSender(), toNano('0.1'), 999n, COLLECTION_TARGET, expiration, 1n, [{ part: 40n }, { part: 40n }, { part: 20n }], [
            {
                price: 100n,
                amount: 1000n
            }
        ]);
        let kickAddress = await kickFactory.getKickContract(COLLECTION_TARGET, expiration, 1n, creator.address, [{ part: 40n }, { part: 40n }, { part: 20n }], [
            {
                price: 100n,
                amount: 1000n
            }
        ...
        kick = blockchain.openContract(Kick.createFromAddress(kickAddress));
    ...
    describe('Kick Contract Tests', () => {
        describe('Back a kick', () => {
            it('should create new Back contract on first backing', async () => {
                ...
                let res = await usdtWallet.sendTransfer(backer.getSender(), toNano("0.1"), amount * price, kick.address, backer.address, Cell.EMPTY, toNano('0.05'), forwardPayload);
                // Verify Jetton balance
            ...
            describe('Resolve kick with Jettons', () => {
                it('should distribute Jettons when milestone is reached', async () => {
                    let collectedKick = blockchain.openContract(await Kick.createFromConfigFull({
        ...
        describe('Vote for kick with Jettons', () => {
            it('should distribute Jettons when votes are passed', async () => {
                ...
                collectedKick = blockchain.openContract(await Kick.createFromConfig({
                    creator: creator.address,
                    target: collected,
                ...
                let forwardPayload = beginCell()
                    ...
                    .storeUint(7, 16)
                    .endCell();
                await usdtWallet.sendTransfer(backer.getSender(), toNano("0.1"), power1, collectedKick.address, backer.address, Cell.EMPTY, toNano('0.05'), forwardPayload);
                let power2 = 400n;
                forwardPayload = beginCell()
                    .storeUint(levelId, 8)
                    .storeUint(4, 16)
                    .endCell();
                await usdtWallet2.sendTransfer(backer2.getSender(), toNano("0.1"), power2, collectedKick.address, backer.address, Cell.EMPTY, toNano('0.05'), forwardPayload);
    ...
    async function deployJettonWallet(wallet) {
        const walletContract = blockchain.openContract(await JettonWallet.createFromAddress(wallet));
        let res = await walletContract.sendDeploy(deployer.getSender(), toNano("0.05"));
        return walletContract;
    }
    async function mintJettons(to, amount) {
        await usdtMaster.sendMint(creator.getSender(), to, amount, toNano('0.05'), toNano('0.1'));
    }
    async function backKickWithAmount(amount) {
        const forwardPayload = beginCell()
            ...
            .storeUint(10n, 16)
            .endCell();
        let res = await usdtWallet.sendTransfer(backer.getSender(), toNano('0.1'), amount, kick.address, backer.address, Cell.EMPTY, toNano('0.05'), forwardPayload);
    }
});
